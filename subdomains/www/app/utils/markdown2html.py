...
def images(match: re.Match):
    img_src = request_ref.url_for("static", path=f"assets/img/{match.group(1)}")
    return f'<img alt="{match.group(1)}" src="{img_src}" class="mx-auto"></img>'
    ...
    for item in list_item_iter:
        ...
        match new_nested_level - nested_level:
            ...
            case -1:  # end of nested list
                list_html = (
                    list_html
                    + f'<li class="{padding_class}">{list_item_content}</li>\n</ol>\n'
                )
...
main_transformations = [
    ...
    {
        "program": re.compile(r"^(\|.*\|)$", re.DOTALL | re.MULTILINE),
        "function": table_greedy,
    },
